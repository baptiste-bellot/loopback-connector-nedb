...
const debug = require('debug')('loopback:connector:nedb');
...
function ObjectID(id) {
  ...
  try {
    // DB's ObjectID constructor accepts number, 12-byte string or 24-byte
...
/**
 * Initialize the NeDB connector for the given data source
...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  s.root = s.root || './tmp';
  s.autoload = s.autoload || false;
  s.stores = Array.isArray(s.stores) ? s.stores : ['Users', 'AccessTokens'];
  dataSource.connector = new NeDB(s, dataSource);
  dataSource.ObjectID = bson.ObjectID;
...
/**
 * The constructor for NeDB connector
 * @param {Object} settings The settings object
 * @param {DataSource} dataSource The data source instance
 * @constructor
 */
function NeDB(settings, dataSource) {
  Connector.call(this, 'nedb', settings);
  ...
  if (
    ...
    NeDB.prototype.findOrCreate = optimizedFindOrCreate;
  }
  if (this.settings.enableGeoIndexing === true) {
    NeDB.prototype.buildNearFilter = buildNearFilter;
  } else {
    NeDB.prototype.buildNearFilter = undefined;
  }
}

util.inherits(NeDB, Connector);
 ...
 */
NeDB.prototype.connect = function (callback) {
  const self = this;
  if (self.db) {
    process.nextTick(function () {
    ...
    const db = {};
    try {
      for (let i = 0; i < self.settings.stores.length; i++) {
        const collection = String(self.settings.stores[i]);
        if (typeof collection !== 'string' || collection === '') continue;
        db[collection] = new nedb({ ...validOptions, filename: path.join(self.settings.root, collection) });
...
NeDB.prototype.getTypes = function () {
  return ['db', 'nosql', 'nedb'];
};

NeDB.prototype.getDefaultIdType = function () {
  return ObjectID;
};
 ...
 */
NeDB.prototype.collectionName = function (modelName) {
  const modelClass = this._models[modelName];
  if (modelClass.settings.nedb) {
    modelName = modelClass.settings.nedb.collection || modelName;
  }
  return modelName;
};

/**
 * Access a NeDB collection by model name
 * @param {String} modelName The model name
 * @returns {*}
 */
NeDB.prototype.collection = function (modelName) {
  if (!this.db) {
    throw new Error(g.f('{{NeDB}} connection is not established'));
  }
 ...
 */
NeDB.prototype.fromDatabase = function (modelName, data) {
  if (!data) {
    return null;
  }
 ...
 */
NeDB.prototype.toDatabase = function (modelName, data) {
...
/**
 * Execute a nedb command
 * @param {String} modelName The model name
 * @param {String} command The command name
 * @param [...] params Parameters for the given command
 */
NeDB.prototype.execute = function (modelName, command) {
  const self = this;
  ...
  if (self.db && self.db.topology && !self.db.topology.isDestroyed()) {
    ...
    self.connect(function (err, db) {
      if (err) {
        debug(
          'Connection not established - NeDB: model=%s command=%s -- error=%s',
          modelName,
          command,
          err,
        );
  ...
  function doExecute() {
    ...
    self.notifyObserversAround(
      ...
      function (context, done) {
        ...
        debug('NeDB: model=%s command=%s', modelName, command, args);
        return collection[context.req.command].apply(collection, args);
      },
      callback,
    );
  }
...
NeDB.prototype.coerceId = function (modelName, id, options) {
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.create = function (modelName, data, options, callback) {
  const self = this;
  if (self.debug) {
  ...
  if (idValue === null) {
    delete data[idName]; // Allow NeDB to generate the id
  } else {
  ...
  this.execute(modelName, 'insertOne', data, function (err, result) {
    if (self.debug) {
    ...
    idValue = result._id || bson.ObjectId.generate();
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.save = function (modelName, data, options, callback) {
  const self = this;
  if (self.debug) {
 ...
 *
 */
NeDB.prototype.exists = function (modelName, id, options, callback) {
  const self = this;
  if (self.debug) {
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.find = function find(modelName, id, options, callback) {
  const self = this;
  if (self.debug) {
...
Connector.defineAliases(NeDB.prototype, 'find', 'findById');
 ...
 * @returns {*}
 */
NeDB.prototype.parseUpdateData = function (modelName, data, options) {
  options = options || {};
  const parsedData = {};
  ...
  if (options.hasOwnProperty('allowExtendedOperators')) {
    ...
    modelClass.settings.nedb &&
    modelClass.settings.nedb.hasOwnProperty('allowExtendedOperators')
  ) {
    allowExtendedOperators =
      modelClass.settings.nedb.allowExtendedOperators === true;
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.updateOrCreate = function updateOrCreate(
  modelName,
  data,
  options,
  callback,
 ...
 */
NeDB.prototype.replaceOrCreate = function (modelName, data, options, cb) {
 ...
 */
NeDB.prototype.destroy = function destroy(modelName, id, options, callback) {
  const self = this;
  if (self.debug) {
...
NeDB.prototype.buildWhere = function (modelName, where, options) {
  const self = this;
  const query = {};
  ...
  Object.keys(where).forEach(function (k) {
    ...
    if (propDef && propDef.nedb && typeof propDef.nedb.dataType === 'string') {
    ...
    if (spec) {
      ...
      if (spec === 'between') {
        ...
        if (cond.global)
          g.warn('{{NeDB}} regex syntax does not respect the {{`g`}} flag');
...
NeDB.prototype.buildSort = function (modelName, order, options) {
  let sort = {};
  const idName = this.idName(modelName);
  ...
  switch (unit) {
    ...
    default:
      console.warn(
        'unsupported unit ' +
        unit +
        ", fallback to NeDB default unit 'meters'",
      );
      return distance;
  }
}
...
function buildNearFilter(query, params) {
  ...
  params.forEach(function (near) {
    ...
    // use NeDB default unit 'meters' rather than 'miles'
...
NeDB.prototype.getDatabaseColumnName = function (model, propName) {
  ...
  // NeDB connector doesn't support custom id property field name
  if (prop.id) {
    // throws if a custom field name for id is set
    const customFieldName = (prop.nedb && (
      prop.nedb.fieldName ||
      prop.nedb.field ||
      prop.nedb.columnName ||
      prop.nedb.column)
    ) ||
      prop.columnName ||
      prop.column;
...
NeDB.prototype.convertColumnNames = function (model, data, direction) {
...
NeDB.prototype.fromPropertyToDatabaseNames = function (model, data) {
  return this.convertColumnNames(model, data, 'database');
};

NeDB.prototype.fromDatabaseToPropertyNames = function (model, data) {
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.all = function all(modelName, filter, options, callback) {
  const self = this;
  if (self.debug) {
  ...
  function processResponse(err, cursor) {
    ...
    return callback(null, self.toModelEntity(modelName, cursor, fields));
 ...
 */
NeDB.prototype.toModelEntity = function (modelName, data, fields) {
  const self = this;
  const idName = self.idName(modelName);
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.destroyAll = function destroyAll(
  modelName,
  where,
  options,
  callback,
 ...
 *
 */
NeDB.prototype.count = function count(modelName, where, options, callback) {
  const self = this;
  if (self.debug) {
 ...
 * @param {Function} [cb] The callback function
 */
NeDB.prototype.replaceById = function replace(modelName, id, data, options, cb) {
 ...
 */
NeDB.prototype.replaceWithOptions = function (modelName, id, data, options, cb) {
  const self = this;
  const idName = self.idName(modelName);
  delete data[idName];
  data = self.toDatabase(modelName, data);
 ...
 * @param {Function} [callback] The callback function
 */
NeDB.prototype.updateAttributes = function updateAttrs(
  modelName,
  id,
  data,
  options,
 ...
 * @callback {Function} cb Callback function
 */
NeDB.prototype.update = NeDB.prototype.updateAll = function updateAll(
  modelName,
  where,
  data,
  options,
  ...
  this.execute(
    ...
    function (err, info) {
      ...
      const affectedCount = info.result ? info.result.n : info;
...
NeDB.prototype.upsertWithWhere = function upsertWithWhere(
  modelName,
  where,
  data,
  options,
  ...
  if (idValue === null || idValue === undefined) {
    delete updateData[idName]; // Allow NeDB to generate the id
  } else {
    const oid = self.coerceId(modelName, idValue, options); // Is it an Object ID?c
...
/**
 * Disconnect from NeDB
 */
NeDB.prototype.disconnect = function (cb) {
  if (this.debug) {
    debug('disconnect');
  }
  /*
 ...
 * @param {Function} [cb] The callback function
 */
NeDB.prototype.autoupdate = function (models, cb) {
  const self = this;
  if (self.db) {
    if (self.debug) {
      debug('autoupdate');
        ...
        for (const p in properties) {
          if (properties[p].index) {
            ...
            if (typeof properties[p].index === 'object') {
              // If there is a nedb key for the index, use it
              if (typeof properties[p].index.nedb === 'object') {
                options = properties[p].index.nedb;
                ...
                // If there isn't an  properties[p].index.nedb object, we read the properties from  properties[p].index
 ...
 * @param {Function} [cb] The callback function
 */
NeDB.prototype.automigrate = function (models, cb) {
  const self = this;
  if (self.db) {
    if (self.debug) {
      debug('automigrate');
    ...
    async.eachSeries(
      ...
      function (modelName, modelCallback) {
        ...
        const opt = { filename: self.db[collectionName].filename };
        delete self.db[collectionName];

        // Recreate the collection
        if (self.debug) {
          debug('create collection %s for model %s', collectionName, modelName);
        }
        self.db[collectionName] = new nedb({ ...opt, autoload: true });
        modelCallback();
      },
      function (err) {
        if (err) {
...
NeDB.prototype.ping = function (cb) {
  const self = this;
  // To Be Redefined
  cb();
};
...
function isStoredAsObjectID(propDef) {
  ...
  if (propDef.nedb) {
    if (ObjectIdTypeRegex.test(propDef.nedb.dataType)) return true;
 ...
 *
 * Only supported on mongodb 2.6+ ? is on nedb ?
 *
 * @param {String} modelName The model name
 * @param {Object} data The model instance data
 * @param {Object} filter The filter
 ...
 * @param {*} propDef Property definition to check if property is for nedb
 */
function coercePropertyValue(modelCtor, propValue, propDef, setValue) {
  let coercedValue;
  // Process only mongo-specific data types
  if (propDef && propDef.nedb && propDef.nedb.dataType) {
    const dataType = propDef.nedb.dataType;
    if (typeof dataType === 'string') {
      if (hasDataType('decimal128', propDef)) {
        if (Array.isArray(propValue)) {
          coercedValue = propValue.map(val => Number(val));
...
function hasDataType(dataType, propertyDef) {
  return propertyDef && propertyDef.nedb &&
    propertyDef.nedb.dataType &&
    propertyDef.nedb.dataType.toLowerCase() === dataType.toLowerCase();
}

/**
* A utility function which adds the connector options to nedb options
* the given data type
